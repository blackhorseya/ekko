// Code generated by Wire. DO NOT EDIT.

//go:generate go run github.com/google/wire/cmd/wire
//go:build !wireinject
// +build !wireinject

package restful

import (
	"github.com/blackhorseya/ekko/app/domain/workflow/biz"
	"github.com/blackhorseya/ekko/app/domain/workflow/repo/issue/mongodb"
	"github.com/blackhorseya/ekko/pkg/adapterx"
	"github.com/blackhorseya/ekko/pkg/authx"
	"github.com/blackhorseya/ekko/pkg/linebotx"
	"github.com/blackhorseya/ekko/pkg/storage/mongodbx"
	"github.com/blackhorseya/ekko/pkg/transports/httpx"
	"github.com/google/wire"
	"github.com/spf13/viper"
)

// Injectors from wire.go:

func New(v *viper.Viper) (adapterx.Servicer, error) {
	server, err := httpx.NewServer()
	if err != nil {
		return nil, err
	}
	messagingApiAPI, err := linebotx.NewClient()
	if err != nil {
		return nil, err
	}
	authenticator, err := authx.NewAuthenticator()
	if err != nil {
		return nil, err
	}
	client, err := mongodbx.NewClient()
	if err != nil {
		return nil, err
	}
	iIssueRepo := mongodb.NewIssueRepo(client)
	iWorkflowBiz := biz.NewWorkflowBiz(iIssueRepo)
	servicer := newService(server, messagingApiAPI, authenticator, iWorkflowBiz)
	return servicer, nil
}

func NewRestful(v *viper.Viper) (adapterx.Restful, error) {
	server, err := httpx.NewServer()
	if err != nil {
		return nil, err
	}
	messagingApiAPI, err := linebotx.NewClient()
	if err != nil {
		return nil, err
	}
	authenticator, err := authx.NewAuthenticator()
	if err != nil {
		return nil, err
	}
	client, err := mongodbx.NewClient()
	if err != nil {
		return nil, err
	}
	iIssueRepo := mongodb.NewIssueRepo(client)
	iWorkflowBiz := biz.NewWorkflowBiz(iIssueRepo)
	restful := newRestful(server, messagingApiAPI, authenticator, iWorkflowBiz)
	return restful, nil
}

// wire.go:

var providerSet = wire.NewSet(httpx.NewServer, linebotx.NewClient, biz.DefaultWorkflowSet, authx.NewAuthenticator)
